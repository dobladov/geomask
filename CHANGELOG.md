# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).


## Unreleased

### Fixed
- Small fix for location save on search

## [1.2.0] - 2025-12-29

### Added
- Changed temp logo with a new one

## [1.1.0] - 2025-12-16

### Added
- UI to save favorite locations
- Save and load zoom level, location name, and distance index in popup
- Animate map to new location when coordinates are set

### Changed
- Rate limit Nominatim requests to 1 per second

### Infrastructure
- Update GitHub Actions to use latest versions of actions

## [1.0.0] - 2025-12-14

### Added
Initial version
